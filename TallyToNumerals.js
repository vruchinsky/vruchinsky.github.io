...
const verticalSpaceBetween50s = 5;
...
function drawBox10(d, x, y, boxWidth) // d is ref. to object of class DrawingOnCanvas (tally)
{
	...
	const ix = boundaryPadding + boundaryThickness; // inner horizontal offset (from the horizontal position of the box)
	const lineHpos = x + ix;
	const iy1 = boundaryPadding + boundaryThickness; // 1st inner vertical offset (from the vertical position of the box)
	let lineVpos = y + iy1;
	const oldlw = d.ctx.lineWidth;
	d.ctx.lineWidth = tallyMarkThickness;
	const columnHeight1 = drawColumnHlines(d, lineHpos, lineVpos, lineLength, 5); // column of five horizontal,
	const iy2 = iy1 + columnHeight1 + verticalSpaceBetween5s; // 2nd inner vertical offset (from the vertical position of the box)
...
function drawColumn50hLines(d, x, y0, yOffset, len) // d is ref. to object of class DrawingOnCanvas (tally)
...
{//...with extra vertical space between each group of 5 and similar column on the right, all enclosed in rectangular box
	...
	const ix1 = boundaryPadding + boundaryThickness; // right column horizontal offset
	const x1 = x + ix1; // left column horizontal position
	const oldlw = d.ctx.lineWidth;
	d.ctx.lineWidth = tallyMarkThickness;
	const yOffset = boundaryPadding + boundaryThickness; // right column vertical offset
	const columnMeasurements = drawColumn50hLines(d, x1, y, yOffset, l);
...
function drawColumn100hLines(d, x, y, xOffset, yOffset, len) // d is ref. to object of class DrawingOnCanvas (tally)
{ // used in drawBox100() and draw500hLinesIn5doubleColumns()
	x += xOffset;
	const columnSize1 = drawColumn50hLines(d, x, y, yOffset, len); // column of 50 horizontal tally marks,
	y += columnSize1.h;
	y += verticalSpaceBetween50s; // extra space halfway down,
	const columnSize2 = drawColumn50hLines(d, x, y, 0, len); // another column of 50 horizontal tally marks underneath
...
{//...each group of 5, and extra space halfway down, and similar column on the right, all enclosed in rectangular box
	...
	const boundingRectWidth = boxWidth - boundaryMargin;
	const l = Math.floor((boundingRectWidth - 2*boundaryThickness - 2*boundaryPadding - hSpaceBetween5s)/2);
	const ix = boundaryPadding + boundaryThickness; // horizontal offset
	const iy = boundaryPadding + boundaryThickness; // vertical offset
	const columnSize = drawColumn100hLines(d, x, y, ix, iy, l);
...
{// used in drawBox1000() and drawBox500()
	...
	const l = Math.floor((dblColWidth - hSpaceBetween5s)/2);
	const dx = dblColWidth + hSpaceBetween100s;
	let xa = new Array(nDblCols);
	let cx = x + xOffset; // left column horizontal position
	xa[0] = xOffset;
	const columnSize = drawColumn100hLines(d, Math.floor(cx), y, 0, yOffset, l);
	for (let i=1; i<nDblCols; i++)
	{
		cx += dx; // move horizontal position
		xa[i] = cx - x;
		drawColumn100hLines(d, Math.floor(cx), y, 0, yOffset, l);
	}
...
{//...space between each group of 5 tally marks, and extra space halfway down, all enclosed in rectangular box
	...
	const ix = boundaryPadding + boundaryThickness; // horizontal offset
	const iy = boundaryPadding + boundaryThickness; // vertical offset
	const columnSize = draw500hLinesIn5doubleColumns(d, x, y, ix, iy, columns500width);
...
//...to look like they occlude each other partially being stacked under each other with a small offset
	...
	let xa = new Array(2);
	xa[0] = boundaryPadding + boundaryThickness; // offset of 500 tally marks on the right
	const oldlw = d.ctx.lineWidth;
	d.ctx.lineWidth = tallyMarkThickness;
	const iy = boundaryPadding + boundaryThickness; // vertical offset
	const columnSize1 = draw500hLinesIn5doubleColumns(d, x, y, xa[0], iy, columns500width);
	xa[1] = xa[0] + columns500width + hSpaceBetween500s; // offset of 500 tally marks on the left
	draw500hLinesIn5doubleColumns(d, x, y, xa[1], iy, columns500width);
	...
	{ //...like they are stacked under the one already drawn but a little offset
		if (i%5==0)
		...
		if (d.calculateOnly == false)
			roundedRect(d.ctx, sx, sy, boundingRectWidth, boundingRectHeight, boxCornerRadius, widthOcclude, heightOcclude);
		d.ctx.strokeStyle = foregroundColor; // restore foreground color
	}
	d.ctx.lineWidth = oldlw; // restore lineWidth
...
{//...1 box of 10 (or 100) tally marks. (corresponds to class MetamorphoseVVtoX (or Fade(romanNumeralsAdditive, "LL", "C")) for numerals)
	...
	fcnDrawing = null; // ref. to function used to draw each of 2 parts of initial graphic
	w1 = 0; // width (in pixels) of each of 2 parts of initial graphic
	...
	yLi = 0; // initial vertical position (in pixels) of left part of initial graphic
	yLf = 0; // final vertical position (in pixels) of left part of initial graphic
	yl = 0; // updated vertical position (in pixels) of left part of initial graphic
	...
	computeGraphicsSizes()
	{
		...
		this.wi = sz.w;
		this.hi = sz.h;
		this.drawingOnCanvas.text = this.finalText; // calculate size of final graphic
		this.drawingOnCanvas.numberOfTallyMarks = this.finalNumber;
		sz = this.fcnDrawing(this.drawingOnCanvas, horizontalOffset, verticalOffset);
		this.wf = sz.w;
		this.hf = sz.h;
...
class ShrinkAndRotateIIIII // the 1st stage of animations of metamorphosis of ||||| into box of 5 horizontal tally marks (the 2nd stage is sliding all the drawings to the right in order to have no empty spaces on the right or in the middle)
{// (corresponds to MetamorphoseIIIIItoV(romanNumeralsAdditive))
	drawingOnCanvas = null; // ref. to DrawingOnCanvas object which contains ref. to HTML canvas object on which to draw the animation and the text to draw
	cnv = null; // HTML canvas object on which to draw the animation
	ctx = null; // drawing context of cnv
	...
	computeGraphicsSizes()
	{
		...
		const oldNumberOfTallyMarks = this.drawingOnCanvas.numberOfTallyMarks;
		this.drawingOnCanvas.calculateOnly = true;
		this.drawingOnCanvas.text = this.initialText; // calculate size of initial graphic
		this.drawingOnCanvas.numberOfTallyMarks = this.initialNumber;
